...
async function getMusicTitlesInLatestSite() {
  ...
  // 2021年代音楽
  await page.goto(
    'https://entamedata.com/2021/12/10/2021%e5%b9%b4%e3%81%ab%e3%83%92%e3%83%83%e3%83%88%e3%81%97%e3%81%9f%e9%9f%b3%e6%a5%bd%e3%83%80%e3%82%a6%e3%83%b3%e3%83%ad%e3%83%bc%e3%83%89%e6%9b%b2%e3%81%a3%e3%81%a6%e4%bd%95%ef%bc%9f/'
  );
  await page.waitForTimeout(1000);

  const trendMusics: TrendMusic[] = [];

  const musicTitleSelector = `#post-22344 > div > small:nth-child(13) > table > tbody > tr:nth-child(n) > td:nth-child(2) > a:nth-child(1)`;
  const musicArtistSelector = `#post-22344 > div > small:nth-child(13) > table > tbody > tr:nth-child(n) > td:nth-child(2)`;
  const musicTitles = await page.$$eval(musicTitleSelector, (elements) =>
    elements.map((element) => element.textContent)
  );
  ...
  for (let i = 0; i < musicTitles.length; i++) {
    const title = musicTitles[i];
    const artist = musicArtists[i];
    trendMusics.push({ trendYear: 2020, title, artist });
  }
